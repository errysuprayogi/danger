...
RSpec.describe Danger::RequestSources::GitLab, host: :gitlab do
  ...
  describe "valid server response" do
    ...
    describe "#update_pull_request!" do
      ...
      context "supports inline comments" do
        ...
        it "ignore new markdown inline when out of range, allow other kind" do
          ...
          v = Danger::Violation.new("Sure thing", true, "Dangerfile", 14)
          m = Danger::Markdown.new("Sure thing", "Dangerfile", 14)
          subject.dismiss_out_of_range_messages = {
            warning: false,
            error: false,
            message: false,
            markdown: true
    ...
    describe "#file_url" do
      ...
      it "returns a valid URL with slug repository" do
        url = subject.file_url(repository: "group/secret_project", organisation: "teapot", path: "Dangerfile")
        expect(url).to eq("https://gitlab.com/api/v4/projects/group%2Fsecret_project/repository/files/Dangerfile/raw?ref=master&private_token=a86e56d46ac78b")
      end
    end
